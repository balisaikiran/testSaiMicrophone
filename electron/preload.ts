...
const electronAPI = {
  ...
  processAudioToText: (audioData: string) => 
    ipcRenderer.invoke("process-audio-to-text", audioData),
  ...
  analyzeScreenshotForPrompt: (screenshotData: string, userPrompt: string) =>
    ipcRenderer.invoke("analyze-screenshot-for-prompt", screenshotData, userPrompt),
  generateCustomResponse: (prompt: string, context: any) =>
    ipcRenderer.invoke("generate-custom-response", prompt, context),
  analyzeMultipleScreenshots: (screenshots: Array<{ data: string; description?: string }>, analysisType: string) =>
    ipcRenderer.invoke("analyze-multiple-screenshots", screenshots, analysisType),
}
